...
func (s *followerServer) Execute(ctx context.Context, req *pb.SQLRequest) (*pb.SQLResponse, error) {
	...
	if _, ok := stmt.(*sqllayer.SelectStatement); !ok {
		return nil, status.Error(codes.PermissionDenied, "follower is read-only")
	}
...
func main() {
	...
	leaderAddr := flag.String("leader", "localhost:5556", "leader replication address")
	port := flag.String("port", "5557", "port to serve read-only SQL on")
	flag.Parse()
	...
	pb.RegisterSQLServiceServer(grpcServer, &followerServer{tm: tm, ex: ex})
	...
	go func() {
		log.Printf("follower SQL server listening on %s (read only)", *port)
		if err := grpcServer.Serve(lis); err != nil {
			log.Printf("grpc serve: %v", err)
		}
	}()

	rc := replication.NewReplicationClient(bt, pm, pm.GetMetaCheckpointLSN(), *leaderAddr, sc.LoadSchemas)
